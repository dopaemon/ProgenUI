...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      try {
        const [summaryResponse, historyResponse, healthResponse, configResponse] = await Promise.all([
          getDashboardSummary(),
          getTrafficHistory(),
          getSystemHealth(),
          getSystemConfig(),
        ...
        <VuiBox mb={3}>
          <Card>
            <VuiBox p={3}>
              ...
              <VuiTypography variant="button" color="text" fontWeight="regular">
                Current Xray configuration generated by the bridge.
              </VuiTypography>
              <VuiBox mt={2}>
                <TextField
                  fullWidth
                  multiline
                  minRows={16}
                  value={systemConfig ? JSON.stringify(systemConfig, null, 2) : "Runtime config unavailable."}
                  InputProps={{
                    readOnly: true,
                  }}
                />
